...
@property(nonatomic,strong)UIView *registerPhoneView;

@property(nonatomic,strong)UIView *registerPassWordView;

@property(nonatomic,strong)UIView *registerSuccessView;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self createRegisterPassWordView];
        [self createRegisterSuccessView];
    }
    return self;
}
...
-(void)createRegisterView
{
    ...
    self.registerPhoneView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kView_width, kView_height)];
    self.registerPhoneView.userInteractionEnabled = YES;
...
//    self.phoneNum.font = font(20);
    [self.registerPhoneView addSubview:self.phoneNum];
...
//    self.codeNum.font = font(20);
    [self.registerPhoneView addSubview:self.codeNum];
...
//    [self.codeBtn setTitleColor:[UIColor colorWithHexString:@"#dbdbdb"] forState:UIControlStateNormal];
    [self.registerPhoneView addSubview:self.codeBtn];
...
//    [self.nextBtn setTitleColor:[UIColor colorWithHexString:@"#fffefe"] forState:UIControlStateNormal];
    ...
    [self.registerPhoneView addSubview:self.nextBtn];
    
}

-(void)createRegisterPassWordView{
    
    self.registerPassWordView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kView_width, kView_height)];
    self.registerPassWordView.userInteractionEnabled = YES;
...
//    self.txtPassWord.font = font(20);
    [self.registerPassWordView addSubview:self.txtPassWord];
...
//    self.aginTxtPassWord.font = font(20);
    [self.registerPassWordView addSubview:self.aginTxtPassWord];
...
//    [sureBtn setTitleColor:[UIColor colorWithHexString:@"#fffefe"] forState:UIControlStateNormal];
    ...
    [self.registerPassWordView addSubview:sureBtn];
    
    
}
-(void)createRegisterSuccessView{
    
    self.registerSuccessView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kView_width, kView_height)];
    self.registerSuccessView.userInteractionEnabled = YES;
    ...
    [self.registerSuccessView addSubview:successViewIV];
    ...
    [self.registerSuccessView addSubview:successLabel];
...
//    [successBtn setTitleColor:[UIColor colorWithHexString:@"#fffefe"] forState:UIControlStateNormal];
    ...
    [self.registerSuccessView addSubview:successBtn];
}


-(void)successBtnSendAction:(UIButton *)sender
{
    //注册结束。关闭注册页。打开登陆页进行登陆
    if ([self.delegate respondsToSelector:@selector(registerRequestEventAction:)]) {
        [self.delegate registerRequestEventAction:sender];
    }
}
...
-(void)passWordsendAction:(UIButton *)sender
{
    ...
    {
        [SVProgressHUD showInfoWithStatus:@"亲,请输入密码号码"];
    ...
    else if (self.txtPassWord.text.length < 6)
    {
        ...
        if ([self.txtPassWord.text isEqualToString:self.aginTxtPassWord.text]) {
            ...
            [self.registerPassWordView removeFromSuperview];
            [self addSubview:self.registerSuccessView];
...
-(UITextField *)codeTxtPassWord
{
    if (!_codeTxtPassWord) {
        ...
        [self.registerPhoneView addSubview:_codeTxtPassWord];
    }
...
-(UIButton *)codeImageBtn{
    ...
    if (!_codeImageBtn) {
        ...
        [self.registerPhoneView addSubview:_codeImageBtn];
    }
...
- (void)showRegisterView
{
    ...
    [self addSubview:self.registerPhoneView];
...
- (void)closeRegisterView
{
    ...
    [self.registerSuccessView removeFromSuperview];
}
